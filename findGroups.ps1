...
foreach($group in [System.IO.File]::ReadLines ("C:/groups.txt"){ #This goes out and reads the file line by line and continues until there is no lines yet
 If ($group -contains $user) { #This functions runs to see if the user in the group line by line 
        Write-Host "$user in the $group group" #This displays an output when the statement is true
